<template>
    <PageHeader :title="$t('mycustomadmin', 'Media')">
        <ImageUploadModal @imageUploaded="imageUploaded">
            <template #button="{ setIsOpen }">
                <Button v-can="'media.upload'" :left-icon="PlusIcon" as="button" @click="() => setIsOpen(true)">
                    {{ $t("mycustomadmin", "Upload media") }}
                </Button>
            </template>
        </ImageUploadModal>
    </PageHeader>
    ...
    <PageContent>
        <Card :no-padding="selectedView === 'list'">
            <template #tabs>
                <Tab :href="route('media.index')" :selected="routeIsActive(route('media.index'))">
                    {{ $t("mycustomadmin", "All") }}
                </Tab>
                <Tab :href="route('media.images')" :selected="routeIsActive(route('media.images'))">
                    {{ $t("mycustomadmin", "Images") }}
                </Tab>
                <Tab :href="route('media.files')" :selected="routeIsActive(route('media.files'))">
                    {{ $t("mycustomadmin", "Files") }}
                </Tab>
            </template>
            <template #actions>
                <div class="flex items-center gap-4 divide-x">
                    <FiltersDropdown :active-filters-count="activeFiltersCount" :reset-filters="resetFilters">
                        <Multiselect v-if="filterOptions.model_type?.length" v-model="filtersForm.model_type"
                            name="model_type" :label="$t('mycustomadmin', 'Model type')"
                            :options="filterOptions.model_type" />
                        <Multiselect v-if="filterOptions.collection_name?.length" v-model="filtersForm.collection_name"
                            name="collection_name" :label="$t('mycustomadmin', 'Collection')"
                            :options="filterOptions.collection_name" />
                        <Multiselect v-if="filterOptions.mime_type?.length" v-model="filtersForm.mime_type"
                            name="mime_type" :label="$t('mycustomadmin', 'Mime type')"
                            :options="filterOptions.mime_type" />
                        <Multiselect v-if="filterOptions.disk?.length" v-model="filtersForm.disk" name="disk"
                            :label="$t('mycustomadmin', 'Disk')" :options="filterOptions.disk" />
                    </FiltersDropdown>
                    <div class="flex items-center gap-4 pl-4">
                        <div class="relative flex gap-1 rounded-md bg-gray-100 p-0.5">
                            <div class="absolute inset-0 right-auto w-1/2 p-0.5 transition-all" :class="{
                                'translate-x-0': selectedView === 'grid',
                                'translate-x-full': selectedView === 'list',
                            }">
                                <div class="h-full w-full rounded-md bg-white shadow-sm" />
                            </div>
                            <IconButton v-for="view in ['grid', 'list']" :key="view" size="xs"
                                :icon="view === 'grid' ? Squares2X2Icon : Bars3Icon"
                                :variant="selectedView === view ? 'ghost' : 'ghost'"
                                :color="selectedView === view ? 'primary' : 'gray'" :class="{
                                    '!bg-transparent': selectedView === view,
                                }" class="z-10" @click="selectedView = view" />
                        </div>
                    </div>
                </div>
            </template>
            ...
            <section>
                ...
                <template v-else>
                    <ul v-if="selectedView === 'grid'" v-auto-animate role="list"
                        ...
                        <li v-for="file in data.data" :key="file.id" class="relative">
                            <div :class="[
                                ...
                                <button type="button" class="absolute inset-0 focus:outline-none"
                                    ...
                                    <span class="sr-only">
                                        {{
                                            $t("mycustomadmin", "View details for :name", {
                    ...
                    <div v-if="selectedView === 'list'" class="overflow-x-auto">
                        <Listing :base-url="baseUrl" :data="data" data-key="data" :with-pagination="false"
                            :with-bulk-select="false" class="inline-block min-w-full align-middle">
                            <template #tableHead>
                                <ListingHeaderCell>
                                    {{ $t("mycustomadmin", "Preview") }}
                                </ListingHeaderCell>

                                <ListingHeaderCell sort-by="title">
                                    {{ $t("mycustomadmin", "Title") }}
                                </ListingHeaderCell>

                                <ListingHeaderCell sort-by="size">
                                    {{ $t("mycustomadmin", "Size") }}
                                </ListingHeaderCell>
                            </template>

                            <template #tableRow="{ item, action }: any">
                                <ListingDataCell @click="selectedImage = item">
                                    <FileThumbnail :file="item" class="!w-14 cursor-pointer" :class="[
                ...
                <div v-auto-animate="{ duration: 100 }" class="space-y-6 pb-16">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="overflow-auto">
                                <h2 class="truncate text-lg font-medium text-gray-900">
                                    <span class="sr-only">{{ $t("mycustomadmin", "Details for") }}
                                    </span>
                                    {{
                    ...
                    <div v-if="showFileInfo">
                        ...
                        <dl class="mt-2 divide-y divide-gray-200 border-t border-b border-gray-200">
                            ...
                            <!-- Custom properties -->
                            <template v-for="(property, key) in selectedImage.custom_properties">
                                <template v-if="key !== 'name'">
                                    <CustomPropertyInput :key="key" v-model="selectedImage.custom_properties[key]"
                                        :propertyName="key" @saved="updateMedia" />
                                </template>
                            </template>
                    ...
                    <div v-if="showFileInfo" class="flex gap-4">
                        <Button as="a" :leftIcon="CloudArrowDownIcon" :href="selectedImage.original_url" :download="selectedImage.custom_properties?.name || selectedImage.file_name
                            ">
                            {{ $t("mycustomadmin", "Download") }}
                        </Button>

                        <Modal type="danger">
                            <template #trigger="{ setIsOpen }">
                                <Button v-can="'media.destroy'" color="gray" variant="outline" :leftIcon="TrashIcon"
                            ...
                            <template #content>
                                {{
                                    $t(
                                        `Are you sure you want to delete this media? It will be permanently removed from our
                                servers and all links pointing to it will be broken. This action cannot be undone.`
                                    )
                                }}
                            </template>

                            <template #buttons="{ setIsOpen }">
                                <Button color="danger" @click.prevent="() => {
...
import { formatBytes } from "@/helpers/index.ts";
