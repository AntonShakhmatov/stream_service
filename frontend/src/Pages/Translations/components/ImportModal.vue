<template>
  <Modal :open="open" @toggle-open="toggleOpen" :external-open="true">
    <template #title>
      {{ $t("mycustomadmin", "Import translations") }}</template
    >
    <template #content>
...
import ImportModalFirstStep from "@/Pages/Translations/components/ImportModalFirstStep.vue";
import ImportModalThirdStep from "@/Pages/Translations/components/ImportModalThirdStep.vue";
import ImportModalSecondStep from "@/Pages/Translations/components/ImportModalSecondStep.vue";
...
const nextStep = () => {
  ...
  if (currentStep.value === 1) {
    let url = route("mycustomadmin.translations.import");
    ...
    axios
      ...
      .then(
        ...
        (error) => {
          toast.error(trans("mycustomadmin", "An error has occurred."));
        }
      )
      .finally(() => {
        processing.value = false;
