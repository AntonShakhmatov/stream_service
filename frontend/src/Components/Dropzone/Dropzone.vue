<template>
  <div class="w-full">
    <label v-if="label" :for="name" class="mb-1 block text-sm font-medium text-gray-700">
      {{ label }}
    </label>

    <div v-auto-animate class="mt-1 rounded-md shadow-sm sm:col-span-2 sm:mt-0">
      <div v-if="filteredFiles.length" class="rounded-md border border-gray-300" :class="{
        'rounded-b-none border-b-0':
          !maxNumberOfFiles || filteredFiles.length < maxNumberOfFiles,
      }">
        <div v-if="onlyThumbs" v-auto-animate class="flex flex-wrap items-center gap-4 p-5"
          :class="{ 'justify-center': centered }">
          <div v-for="file in filteredFiles" :key="file.uuid" class="group relative">
            ...
            <label :for="file.uuid" type="button"
              ...
              <input :id="file.uuid" class="sr-only absolute" type="file" :name="file.uuid" multiple="false"
                :accept="accept" @input="switchFile(file.uuid, $event)" />
            </label>
            <button type="button"
        ...
        <div v-else v-auto-animate class="divide-y divide-gray-100 border-gray-300">
          <div v-for="file in filteredFiles" :key="file.uuid"
            ...
            <div class="group relative">
              ...
              <div
                ...
                <label :for="file.uuid" type="button">
                  <Button :left-icon="ArrowPathRoundedSquareIcon" size="sm" color="gray" variant="outline" as="span">
                    <!-- {{ $t("mycustomadmin", "Replace") }} -->
                    {{ $t("Replace") }}
                  </Button>
                ...
                <Modal v-if="!file.id" size="lg">
                  <template #trigger="{ setIsOpen }">
                    <Button size="sm" color="gray" variant="outline" :left-icon="ArrowsPointingInIcon"
                      @click="() => setIsOpen(true)">
                      <!-- {{ $t("mycustomadmin", "Crop") }} -->
                      {{ $t("Crop") }}
                    </Button>
                  </template>
                  <template #content="{ setIsOpen }">
                    <ImageEditor :src="file.base64" @onCrop="(croppedFile, path) => {
            ...
            <div class="col-span-2 space-y-3">
              <slot name="inputs" :file="file" :set-custom-property="setCustomProperty"
                :get-custom-property="getCustomProperty">
                <TextInput v-model="file.custom_properties.name" size="sm" label="File name"
                  :name="`${file.uuid ?? file.id}_name`" />
                <TextInput v-model="file.custom_properties.alt" size="sm" label="Alt text"
                  :name="`${file.uuid ?? file.id}_alt`" />
              </slot>
            </div>
            <button type="button" class="absolute right-3 top-2 rounded-full text-gray-500 hover:text-gray-700"
              @click="removeImage(file)">
      ...
      <label v-if="!maxNumberOfFiles || filteredFiles.length < maxNumberOfFiles" :for="name"
        class="relative block shrink-0 cursor-pointer rounded-b-md border border-dashed p-5" :class="{
          'rounded-t-md': !filteredFiles.length,
          'border-primary-400': dropIsActive,
          'hover:border-primary-400 border-gray-300': !dropIsActive,
        }" @dragover.prevent="dragOver" @dragleave.prevent="dragLeave" @drop.prevent="onFilesDrop">
        <input :id="name" class="sr-only absolute" type="file" :name="name" :multiple="multiple" :accept="accept"
          @input="onFilesSelect" />
        <div class="flex items-center gap-2" :class="{ 'flex-col text-center': centered }">
          <slot name="placeholder" :max-number-of-files="maxNumberOfFiles" :formated-accept="formatedAccept"
            :max-file-size="maxFileSize" :format-bytes="formatBytes">
            <slot name="icon">
...
import { formatBytes, getFileExtension } from "@/helpers/index.ts";
import { trans } from "@/plugins/laravel-vue-i18n";
...
const uploadFiles = (filesToUpload: File[]) => {
  ...
  filesToUpload.forEach((file: File) => {
    if (file.size > props.maxFileSize!) {
      toast.error(trans("mycustomadmin", "File size is too big!"));
      return;
    }
