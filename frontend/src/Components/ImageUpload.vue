<template>
  <div>
    <label v-if="label" :for="name" class="text-blue-gray-900 mb-1 block text-sm font-medium">
      {{ label }}</label>
    <div class="flex items-center gap-4">
      <label :for="name" class="text-blue-gray-900 relative cursor-pointer text-sm font-medium"
        @dragover.prevent="dragOver" @dragleave.prevent="dragLeave" @drop.prevent="onFilesDrop">
        <Avatar size="xl" :src="filteredFiles?.[0]?.preview_url ?? filteredFiles?.[0]?.base64" />
      </label>
      <div class="flex gap-4">
        <Button variant="outline" color="gray" class="relative">
          <label :for="name" class="text-blue-gray-900 relative cursor-pointer text-sm font-medium"
            @dragover.prevent="dragOver" @dragleave.prevent="dragLeave" @drop.prevent="onFilesDrop">
            <!-- <span>{{ $t("mycustomadmin", "Change") }}</span> -->
            <span>{{ $t("Change") }}</span>
          </label>
        ...
        <Button variant="ghost" color="primary" @click="removeImage(files?.[0])">
          <!-- {{ $t("mycustomadmin", "Remove") }} -->
          {{ $t("Remove") }}
        </Button>
      </div>
    </div>
...
const uploadFile = (file: File) => {
  if (file.size > props.maxFileSize!) {
    // toast.error(
    //   trans("mycustomadmin", "File size is too big!")
    //   // trans("mycustomadmin", "validation.max.file", {
    //   //   attribute: "obrázok",
